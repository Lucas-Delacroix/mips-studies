.data 
	array:
		...
		.space 40 # Aloca 4 espaços no array
	space:
 ...
 .text
	...
	move $t1, $zero
	li $t2, 40 # Define Tamanho do array (16/4) elementos

	loopCriarArray:
		beq $t0, $t2, loopImprimirDesordenado #Se o indice for maior que a lista sai do loop
	...
	loopOrdenar:
		li $v0, 4 #Adiciona imprimir string ao v0
		la $a0, newline #Variavel que contem o \n
		syscall 
		move $t3, $zero #indice do array
	
	ordena:
		beq $t3, $t2, loopImprimirOrdenado #Se o indice for maior que a lista sai do loop
